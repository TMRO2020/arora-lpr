...
void OptionsModel::setDisplayDefaultOptions(QSettings& settings, bool reset)
{
    if (!settings.contains("nDisplayUnit") || reset)
        settings.setValue("nDisplayUnit", BitcoinUnits::LPR);
    ...
    if (settings.contains("nAnonymizeAroraAmount") || reset)
        SoftSetArg("-anonymizelpramount", settings.value("nAnonymizeAroraAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in arora.cpp during shotdown to also remove the window positions
