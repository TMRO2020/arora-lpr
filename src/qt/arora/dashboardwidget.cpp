...
#include "qt/arora/dashboardwidget.h"
#include "qt/arora/forms/ui_dashboardwidget.h"
#include "qt/arora/sendconfirmdialog.h"
#include "qt/arora/txrow.h"
...
DashboardWidget::DashboardWidget(AroraGUI* parent) :
    PWidget(parent),
    ui(new Ui::DashboardWidget)
{
    ...
    setCssProperty(ui->labelSquareLPR, "square-chart-lpr");
    setCssProperty(ui->labelLPR, "text-chart-lpr");
    ...
    setCssProperty(ui->labelAmountLPR, "text-stake-lpr-disable");
...
bool hasCharts = false;
    ...
    if (window)
        connect(window, &AroraGUI::windowResizeEvent, this, &DashboardWidget::windowResizeEvent);
#endif
...
void DashboardWidget::loadWalletModel()
{
    if (walletModel && walletModel->getOptionsModel()) {
        ...
        connect(ui->pushImgEmpty, &QPushButton::clicked, window, &AroraGUI::openFAQ);
        connect(ui->btnHowTo, &QPushButton::clicked, window, &AroraGUI::openFAQ);
...
bool DashboardWidget::loadChartData(bool withMonthNames)
{
    ...
    for (int j = range.first; j < range.second; j++) {
        ...
        if (chartData->amountsByCache.contains(num)) {
            ...
            lpr = (pair.first != 0) ? pair.first / 100000000 : 0;
            chartData->totalLPR += pair.first;
        }

        chartData->xLabels << ((withMonthNames) ? monthsNames[num - 1] : QString::number(num));

        chartData->valuesLPR.append(lpr);

        int max = std::max(lpr, lpr);
...
void DashboardWidget::onChartRefreshed()
{
    ...
    set0->append(chartData->valuesLPR);

    // Total
    nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();
    if (chartData->totalLPR > 0) {
        setCssProperty(ui->labelAmountLPR, "text-stake-lpr");
    } else {
        setCssProperty(ui->labelAmountLPR, "text-stake-lpr-disable");
    }
    forceUpdateStyle({ui->labelAmountLPR});
    ui->labelAmountLPR->setText(GUIUtil::formatBalance(chartData->totalLPR, nDisplayUnit));
