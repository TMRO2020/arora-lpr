...
#include "qt/arora/forms/ui_snackbar.h"
#include "qt/arora/qtutils.h"
#include <QTimer>


SnackBar::SnackBar(AroraGUI* _window, QWidget *parent) :
    QDialog(parent),
    ui(new Ui::SnackBar),
    window(_window),
    timeout(MIN_TIMEOUT)
{
    ...
    if (window)
        connect(window, &AroraGUI::windowResizeEvent, this, &SnackBar::windowResizeEvent);
    else {
