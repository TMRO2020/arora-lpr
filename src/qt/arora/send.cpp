...
#include "qt/arora/forms/ui_send.h"
#include "qt/arora/addnewcontactdialog.h"
#include "qt/arora/qtutils.h"
#include "qt/arora/sendchangeaddressdialog.h"
#include "qt/arora/optionbutton.h"
#include "qt/arora/sendconfirmdialog.h"
#include "qt/arora/myaddressrow.h"
#include "qt/arora/guitransactionsutils.h"
...
SendWidget::SendWidget(AroraGUI* parent) :
    PWidget(parent),
    ui(new Ui::send),
    ...
    btnContacts(new QPushButton())
{
    ...
    setCssProperty(coinIcon, "coin-icon-lpr");
...
void SendWidget::onContactMultiClicked()
{
    if (focusedEntry) {
        ...
        CTxDestination lprAdd = DecodeDestination(address.toStdString());
        if (walletModel->isMine(lprAdd)) {
        ...
        if (dialog->res) {
            ...
            if (walletModel->updateAddressBookLabels(lprAdd, dialog->getLabel().toStdString(),
                    AddressBook::AddressBookPurpose::SEND)) {
