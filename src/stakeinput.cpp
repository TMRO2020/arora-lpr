...
bool CLPRStake::InitFromTxIn(const CTxIn& txin)
{
...
bool CLPRStake::SetPrevout(CTransaction txPrev, unsigned int n)
{
    this->txFrom = txPrev;
    this->nPosition = n;
    return true;
...
bool CLPRStake::GetTxFrom(CTransaction& tx) const
{
    if (txFrom.IsNull())
        return false;
    tx = txFrom;
...
bool CLPRStake::GetTxOutFrom(CTxOut& out) const
{
...
bool CLPRStake::CreateTxIn(CWallet* pwallet, CTxIn& txIn, uint256 hashTxOut)
{
    txIn = CTxIn(txFrom.GetHash(), nPosition);
    return true;
}

CAmount CLPRStake::GetValue() const
{
    return txFrom.vout[nPosition].nValue;
}

bool CLPRStake::CreateTxOuts(CWallet* pwallet, std::vector<CTxOut>& vout, CAmount nTotal)
{
...
CDataStream CLPRStake::GetUniqueness() const
{
    //The unique identifier for a LPR stake is the outpoint
...
CBlockIndex* CLPRStake::GetIndexFrom()
{
    if (pindexFrom)
        return pindexFrom;
...
bool CLPRStake::ContextCheck(int nHeight, uint32_t nTime)
{
