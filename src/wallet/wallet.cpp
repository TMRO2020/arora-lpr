...
/**
 * Fees smaller than this (in ulpr) are considered zero fee (for transaction creation)
    ...
    const std::vector<unsigned char>& vchCryptedSecret)
{
    ...
    {
        LOCK(cs_wallet);
        if (pwallelprEncryption)
            return pwallelprEncryption->WriteCryptedKey(vchPubKey,
                vchCryptedSecret,
...
bool CWallet::SetMinVersion(enum WalletFeature nVersion, CWalletDB* pwallelprIn, bool fExplicit)
{
    ...
    if (fFileBacked) {
        CWalletDB* pwallelpr = pwallelprIn ? pwallelprIn : new CWalletDB(strWalletFile);
        if (nWalletVersion > 40000)
            pwallelpr->WriteMinVersion(nWalletVersion);
        if (!pwallelprIn)
            delete pwallelpr;
    }
...
bool CWallet::EncryptWallet(const SecureString& strWalletPassphrase)
{
...

    {
        ...
        if (fFileBacked) {
            assert(!pwallelprEncryption);
            pwallelprEncryption = new CWalletDB(strWalletFile);
            if (!pwallelprEncryption->TxnBegin()) {
                delete pwallelprEncryption;
                pwallelprEncryption = NULL;
                return false;
            }
            pwallelprEncryption->WriteMasterKey(nMasterKeyMaxID, kMasterKey);
        }

        if (!EncryptKeys(vMasterKey)) {
            if (fFileBacked) {
                pwallelprEncryption->TxnAbort();
                delete pwallelprEncryption;
            }
        ...
        SetMinVersion(FEATURE_WALLETCRYPT, pwallelprEncryption, true);

        if (fFileBacked) {
            if (!pwallelprEncryption->TxnCommit()) {
                delete pwallelprEncryption;
...
int64_t CWallet::IncOrderPosNext(CWalletDB* pwallelpr)
{
    AssertLockHeld(cs_wallet); // nOrderPosNext
    int64_t nRet = nOrderPosNext++;
    if (pwallelpr) {
        pwallelpr->WriteOrderPosNext(nOrderPosNext);
    } else {
...
bool CWallet::AddToWallet(const CWalletTx& wtxIn, bool fFromLoadWallet, CWalletDB* pwallelpr)
{
    ...
    if (fFromLoadWallet) {
        ...
        if (fInsertedNew) {
            ...
            wtx.nOrderPos = IncOrderPosNext(pwallelpr);
            wtxOrdered.insert(std::make_pair(wtx.nOrderPos, TxPair(&wtx, (CAccountingEntry*)0)));
            wtx.UpdateTimeSmart();
            AddToSpends(hash);
        }
...
bool CWalletTx::WriteToDisk(CWalletDB *pwallelpr)
{
    if (pwallelpr)
        return pwallelpr->WriteTx(GetHash(), *this);
...
bool CWallet::GetBudgetSystemCollateralTX(CWalletTx& tx, uint256 hash, bool useIX)
{
    ...
    vecSend.push_back(std::make_pair(scriptChange, BUDGET_FEE_TX_OLD)); // Old 50 LPR collateral
...
bool CWallet::GetBudgetFinalizationCollateralTX(CWalletTx& tx, uint256 hash, bool useIX)
{
    ...
    std::string strFail = "";
    std::vector<std::pair<CScript, CAmount> > vecSend;
    vecSend.push_back(std::make_pair(scriptChange, BUDGET_FEE_TX)); // New 5 LPR collateral
        ...
        LOCK2(cs_main, cs_wallet);
        {
            ...
            while (true) {
                ...
                if (nChange > 0) {
                    ...
                    // change transaction isn't always pay-to-arora-address
                    CScript scriptChange;
...
{
    {
        ...
        LogPrintf("CommitTransaction:\n%s", wtxNew.ToString());
        {
            ...
            CWalletDB* pwallelpr = fFileBacked ? new CWalletDB(strWalletFile, "r+") : NULL;
            ...
            AddToWallet(wtxNew, false, pwallelpr);
...
bool CWallet::AddAccountingEntry(const CAccountingEntry& acentry, CWalletDB & pwallelpr)
{
    if (!pwallelpr.WriteAccountingEntry_Backend(acentry))
        return false;
