...
void ThreadFlushWalletDB(const std::string& strFile)
{
    ...
    util::ThreadRename("arora-wallet");
    ...
    while (true) {
        ...
        if (nLastFlushed != nWalletDBUpdated && GetTime() - nLastWalletUpdate >= 2) {
            TRY_LOCK(bilpr.cs_db, lockDb);
            if (lockDb) {
                // Don't do this if any databases are in use
                int nRefCount = 0;
                std::map<std::string, int>::iterator mi = bilpr.mapFileUseCount.begin();
                while (mi != bilpr.mapFileUseCount.end()) {
                    nRefCount += (*mi).second;
                    mi++;
                }
                ...
                if (nRefCount == 0) {
                    ...
                    std::map<std::string, int>::iterator mi = bilpr.mapFileUseCount.find(strFile);
                    if (mi != bilpr.mapFileUseCount.end()) {
                        LogPrint(BCLog::DB, "Flushing wallet.dat\n");
                        nLastFlushed = nWalletDBUpdated;
                        int64_t nStart = GetTimeMillis();

                        // Flush wallet.dat so it's self contained
                        bilpr.CloseDb(strFile);
                        bilpr.CheckpointLSN(strFile);

                        bilpr.mapFileUseCount.erase(mi++);
...
bool BackupWallet(const CWallet& wallet, const fs::path& strDest, bool fEnableCustom)
{
    ...
    while (true) {
        {
            LOCK(bilpr.cs_db);
            if (!bilpr.mapFileUseCount.count(wallet.strWalletFile) || bilpr.mapFileUseCount[wallet.strWalletFile] == 0) {
                // Flush log data to the dat file
                bilpr.CloseDb(wallet.strWalletFile);
                bilpr.CheckpointLSN(wallet.strWalletFile);
                bilpr.mapFileUseCount.erase(wallet.strWalletFile);
