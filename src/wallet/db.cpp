...
CDB::CDB(const std::string& strFilename, const char* pszMode, bool fFlushOnCloseIn) : pdb(NULL), activeTxn(NULL)
{
...

    {
        ...
        strFile = strFilename;
        ++bilpr.mapFileUseCount[strFile];
        pdb = bilpr.mapDb[strFile];
        if (pdb == NULL) {
            pdb = new Db(bilpr.dbenv, 0);

            bool fMockDb = bilpr.IsMock();
            if (fMockDb) {
...
void CDB::Flush()
{
    ...
    bilpr.dbenv->txn_checkpoint(nMinutes ? GetArg("-dblogsize", 100) * 1024 : 0, nMinutes, 0);
}
...
bool CDB::Rewrite(const std::string& strFile, const char* pszSkip)
{
    while (true) {
        {
            LOCK(bilpr.cs_db);
            if (!bilpr.mapFileUseCount.count(strFile) || bilpr.mapFileUseCount[strFile] == 0) {
                // Flush log data to the dat file
                bilpr.CloseDb(strFile);
                bilpr.CheckpointLSN(strFile);
                bilpr.mapFileUseCount.erase(strFile);
                ...
                { // surround usage of db with extra {}
                    ...
                    Db* pdbCopy = new Db(bilpr.dbenv, 0);
                ...
                if (fSuccess) {
                    Db dbA(bilpr.dbenv, 0);
                    if (dbA.remove(strFile.c_str(), NULL, 0))
                        fSuccess = false;
                    Db dbB(bilpr.dbenv, 0);
