...
void PrepareShutdown()
{
    ...
    util::ThreadRename("arora-shutoff");
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), "arora.conf"));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), "lprd.pid"));
#endif
    ...
    strUsage += HelpMessageOpt("-port=<port>", strprintf(_("Listen for connections on <port> (default: %u or testnet: %u)"), 23201, 23203));
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all Arora specific functionality (Masternodes, SwiftX, Budgeting) (0-1, default: %u)"), 0));
    ...
    strUsage += HelpMessageOpt("-minstakesplit=<amt>", strprintf(_("Minimum positive amount (in LPR) allowed by GUI and RPC for the stake split threshold (default: %s)"),
    ...
    strUsage += HelpMessageOpt("-masternodeaddr=<n>", strprintf(_("Set external address:port to get to this masternode (example: %s)"), "128.127.106.235:23201"));
    ...
    strUsage += HelpMessageOpt("-rpcport=<port>", strprintf(_("Listen for JSON-RPC connections on <port> (default: %u or testnet: %u)"), 23202, 23204));
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2015-%i The Arora developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
{
    util::ThreadRename("arora-loadblk");
...
/** Sanity checks
 *  Ensure that Arora is running in a usable environment with all
...
// ********************************************************* Step 1: setup
    ...
    _CrtSetReportMode(_CRT_WARN, _CRLPRG_MODE_FILE);
...
bool AppInit2()
{
 ...
 /*   if (GetBoolArg("-masternode", false) && !GetBoolArg("-listen", true))
    ...
    // Sanity check
    if (!InitSanityCheck())
        return UIError(_("Initialization sanity check failed. Arora is shutting down."));
...
#endif
    // Make sure only a single Arora process is using the data directory.
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return UIError(strprintf(_("Cannot obtain a lock on data directory %s. Arora is probably already running."), strDataDir));
...
// ********************************************************* Step 5: Backup wallet and verify wallet database integrity
    ...
    if (!fDisableWallet) {
        ...
        if (GetBoolArg("-salvagewallet", false)) {
            ...
            if (!CWalletDB::Recover(bilpr, strWalletFile, true))
                return false;
        }

        if (fs::exists(GetDataDir() / strWalletFile)) {
            CDBEnv::VerifyResult r = bilpr.Verify(strWalletFile, CWalletDB::Recover);
    ...
    while (!fLoaded && !ShutdownRequested()) {
        ...
        do {
            ...
            try {
                ...
                // Arora: load previous sessions sporks if we have them.
...
// ********************************************************* Step 8: load wallet
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK) {
            if (nLoadWalletRet == DB_CORRUPT)
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of Arora") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE) {
                strErrors << _("Wallet needed to be rewritten: restart Arora to complete") << "\n";
        ...
        if (GetBoolArg("-upgradewallet", fFirstRun && !fLegacyWallet)) {
            if (prev_version <= FEATURE_PRE_Arora && pwalletMain->IsLocked()) {
        ...
        if (fFirstRun) {
            if (!fLegacyWallet) {
                ...
                pwalletMain->SetMaxVersion(FEATURE_PRE_Arora);
            }
    ...
    CBudgetDB budgelpr;
    CBudgetDB::ReadResult readResult2 = budgelpr.Read(budget);
