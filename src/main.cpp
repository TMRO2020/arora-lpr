...
#if defined(NDEBUG)
#error "Arora cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in ulpr) are considered zero fee (for relaying, mining and transaction creation)
...
bool RecalculateLPRSupply(int nHeightStart)
{
	AssertLockHeld(cs_main);
	...
	uiInterface.ShowProgress(_("Recalculating LPR supply..."), 0);
	while (true) {
		if (pindex->nHeight % 1000 == 0) {
			LogPrintf("%s : block %d...\n", __func__, pindex->nHeight);
			int percent = std::max(1, std::min(99, (int)((double)((pindex->nHeight - nHeightStart) * 100) / (chainHeight - nHeightStart))));
			uiInterface.ShowProgress(_("Recalculating LPR supply..."), percent);
		}
...
{
    util::ThreadRename("arora-scriptch");
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
    ...
    if (isPoS) {
        ...
        // Inputs
        std::vector<CTxIn> lprInputs;

        for (const CTxIn& stakeIn : stakeTxIn.vin) {
                lprInputs.push_back(stakeIn);
        }
        const bool hasLPRInputs = !lprInputs.empty();
        ...
        for (const CTransaction& tx : block.vtx) {
            for (const CTxIn& in: tx.vin) {
                ...
                if(hasLPRInputs) {
                    ...
                    for (const CTxIn& lprIn : lprInputs)
                        if(lprIn.prevout == in.prevout)
            ...
            while (!chainActive.Contains(prev)) {
                ...
                for (const CTransaction& t : bl.vtx) {
                    ...
                    for (const CTxIn& in: t.vin) {
                        ...
                        if (hasLPRInputs) {
                            for (const CTxIn& stakeIn : lprInputs)
...
bool static ProcessMessage(CNode* pfrom, std::string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == NetMsgType::VERSION) {
        ...
        // Arora: We use certain sporks during IBD, so check to see if they are
