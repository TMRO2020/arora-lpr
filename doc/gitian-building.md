...
*Setup instructions for a gitian build of Arora using a VM or physical system.*

Gitian is the deterministic build process that is used to build the Arora
...
to the Arora GitHub release page.
...
- [Building Arora](#building-arora-core)
...
```bash
gpg --output ${VERSION}-linux/${NAME}/arora-linux-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-linux/$NAME/arora-linux-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-osx-unsigned/$NAME/arora-osx-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-osx-unsigned/$NAME/arora-osx-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-win-unsigned/$NAME/arora-win-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-win-unsigned/$NAME/arora-win-${VERSION%\.*}-build.assert
```

Uploading Signatures
--------------------
Make a Pull Request (both the `.assert` and `.assert.sig` files) to the
[gitian.sigs](https://github.com/arora-project/gitian.sigs/) repository:
...
You can also mail the files to Fuzzbawls (fuzzbawls@lprcoin.com) and he will commit them.

```bash
gpg --detach-sign ${VERSION}-linux/${NAME}/arora-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${NAME}/arora-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${NAME}/arora-osx-*-build.assert
```
